...
type SubMenuProps = { hoveredModelData: Model }
    ...
    <div className="bg-popover border-border w-[280px] rounded-md border p-3 shadow-md">
      <div className="flex flex-col gap-2">
        <div className="flex items-center gap-3">
          {hoveredModelData?.icon && (<hoveredModelData.icon className="size-5" />)}
          <h3 className="font-medium">{hoveredModelData.name}</h3>
        </div>
        <p className="text-muted-foreground text-sm">{hoveredModelData.description}</p>
        <div className="flex flex-col gap-1">
          <div className="mt-1 flex flex-wrap gap-2">
            {hoveredModelData.features?.find((f: any) => f.id === "file-upload")?.enabled && (
            ...
            {hoveredModelData.features?.find((f: any) => f.id === "tool-use")?.enabled && (
            ...
            {hoveredModelData.features?.find((f: any) => f.id === "reasoning")?.enabled && (
        ...
        <div className="flex flex-col gap-2">
          <div className="flex justify-between text-sm"><span className="font-medium">Provider</span><span>{hoveredModelData.provider}</span></div>
          <div className="flex justify-between text-sm"><span className="text-sm font-medium">Id</span><span className="text-muted-foreground text-xs">{String(hoveredModelData.id)}</span></div>
        </div>
      </div>
    </div>
  )
