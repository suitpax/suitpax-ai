...
export type TextMorphProps = { children: string; as?: React.ElementType; className?: string; style?: React.CSSProperties; variants?: Variants; transition?: Transition }
...
export function TextMorph({ children, as: Component = "p", className, style, variants, transition }: TextMorphProps) {
  ...
  const defaultVariants: Variants = { initial: { opacity: 0 }, animate: { opacity: 1 }, exit: { opacity: 0 } }
  const defaultTransition: Transition = { type: "spring", stiffness: 280, damping: 18, mass: 0.3 }
