...
import { Search, Loader2, AlertCircle, Filter, CalendarIcon, UsersIcon, Briefcase, Badge } from "lucide-react"
...
import { NotebookTabsIcon as Tabs } from "lucide-react"
...
export default function FlightsPage() {
  ...
  const validateForm = (): boolean => {
    ...
    const totalPassengers =
      searchForm.passengers.adults + searchForm.passengers.children + searchForm.passengers.infants
  ...
  const applyClientFilters = (filters: typeof activeFilters) => {
    const filtered = offers.filter((offer) => {
      ...
      if (filters.airlines.length > 0) {
        const hasAirline = offer.slices.every((slice) =>
          slice.segments.some((seg) => filters.airlines.includes(seg.marketing_carrier?.iata_code)),
        )
  ...
  const isFormValid =
    searchForm.origin && searchForm.destination && searchForm.departure_date && Object.keys(formErrors).length === 0
    ...
    <div className="min-h-full bg-gradient-to-br from-gray-50 to-gray-100">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6">
        <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
          ...
          <div className="flex gap-4">
            <div className="bg-white/70 backdrop-blur-sm rounded-2xl p-4 border border-gray-200">
              ...
              <p className="text-xs text-gray-600 mt-1">Updated hourly</p>
            </div>
            ...
            <div className="bg-white/70 backdrop-blur-sm rounded-2xl p-4 border border-gray-200">
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm font-medium">Premium Routes</span>
              </div>
              <p className="text-xs text-gray-600 mt-1">Business class</p>
            </div>
          </div>
        </div>

        <Card className="rounded-3xl border border-gray-200 bg-white/70 backdrop-blur-sm shadow-sm hover:shadow-md">
          <CardHeader>
          ...
          <CardContent className="space-y-6">
            <Tabs defaultValue="flights" className="w-full">
              <Tabs.List className="grid w-full grid-cols-3 bg-gray-100">
                <Tabs.Trigger value="flights">Flights</Tabs.Trigger>
                <Tabs.Trigger value="multi-city">Multi-city</Tabs.Trigger>
                <Tabs.Trigger value="flexible">Flexible Dates</Tabs.Trigger>
              </Tabs.List>

              <Tabs.Content value="flights" className="space-y-6 mt-6">
                {/* General Error */}
                {formErrors.general && (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div className="relative">
                    ...
                    />
                    {formErrors.return_date && <p className="mt-1 text-xs text-red-500">{formErrors.return_date}</p>}
                  </div>
                </div>
                  ...
                  <div className="relative">
                    ...
                    <Select
                      ...
                      onValueChange={(value) =>
                        setSearchForm((prev) => ({
                          ...prev,
                          passengers: { ...prev.passengers, adults: Number.parseInt(value) },
                        }))
                      }
                    >
                      <Select.Trigger className="pl-9 rounded-full border-gray-300/60 bg-white/70 backdrop-blur-sm">
                        <Select.Value />
                      </Select.Trigger>
                      <Select.Content>
                        {[...Array(9)].map((_, i) => (
                  ...
                  <div className="relative">
                    ...
                    <Select
                      ...
                      onValueChange={(value) =>
                        setSearchForm((prev) => ({
                          ...prev,
                          passengers: { ...prev.passengers, children: Number.parseInt(value) },
                        }))
                      }
                    >
                      <Select.Trigger className="pl-9 rounded-full border-gray-300/60 bg-white/70 backdrop-blur-sm">
                  ...
                  <div className="relative">
                    ...
                    <Select
                      ...
                      onValueChange={(value) =>
                        setSearchForm((prev) => ({
                          ...prev,
                          passengers: { ...prev.passengers, infants: Number.parseInt(value) },
                        }))
                      }
                    >
                      <Select.Trigger className="pl-9 rounded-full border-gray-300/60 bg-white/70 backdrop-blur-sm">
                ...
                <Button
                  ...
                  className="w-full bg-black hover:bg-gray-800 text-white rounded-full py-6 text-base font-medium tracking-tight transition-all duration-200 hover:shadow-lg"
                >
              ...
              <Tabs.Content value="multi-city" className="space-y-6 mt-6">
                <div className="text-center py-8">
                  <Badge variant="outline" className="border-blue-200 text-blue-800">
                    Enterprise
                  </Badge>
                  <h3 className="text-lg font-medium mb-2">Multi-city Search</h3>
                  <p className="text-sm text-gray-600">Coming soon - Plan complex itineraries</p>
                </div>
              </Tabs.Content>

              <Tabs.Content value="flexible" className="space-y-6 mt-6">
                <div className="text-center py-8">
                  <Badge variant="outline" className="border-blue-200 text-blue-800">
                    Enterprise
                  </Badge>
                  <h3 className="text-lg font-medium mb-2">Flexible Dates</h3>
                  <p className="text-sm text-gray-600">Coming soon - Find the best deals across dates</p>
                </div>
              </Tabs.Content>
            </Tabs>
          </CardContent>
        ...
        {searchPerformed && (
          <div>
            {isLoading ? (
              <Card className="rounded-3xl border border-gray-200 bg-white/70 backdrop-blur-sm">
                <CardContent className="flex justify-center items-center py-12">
                  <div className="text-center">
                    <Loader2 className="h-8 w-8 animate-spin mx-auto mb-4 text-gray-600" />
                    <p className="text-gray-600 font-medium">Searching for flights...</p>
                    <p className="text-sm text-gray-500 mt-1">Checking 500+ airlines worldwide</p>
                  </div>
                </CardContent>
              </Card>
            ) : offers.length > 0 ? (
              <>
                <Card className="rounded-3xl border border-gray-200 bg-white/70 backdrop-blur-sm mb-6">
                    ...
                    <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4">
                      <div>
                        ...
                        <div className="text-sm text-gray-500 flex items-center gap-4 mt-1">
                          <span>{searchForm.return_date ? "Round trip" : "One way"}</span>
                          <span>•</span>
                          <span>
                            {searchForm.passengers.adults +
                              searchForm.passengers.children +
                              searchForm.passengers.infants}{" "}
                            passenger
                            {searchForm.passengers.adults +
                              searchForm.passengers.children +
                              searchForm.passengers.infants !==
                      ...
                      <Button
                        ...
                        className="flex items-center gap-2 bg-white/80 text-gray-900 border-gray-200 hover:bg-gray-50 rounded-full"
                      >
                ...
                <div className="space-y-4">
                  {filteredOffers.map((offer) => (
                    <FlightCard key={offer.id} offer={offer} onSelect={handleSelectFlight} />
                  ))}
                </div>
