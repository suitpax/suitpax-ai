...
import { Message as PKMessage, MessageContent as PKMessageContent, MessageActions as PKMessageActions, MessageAction as PKMessageAction } from "@/components/prompt-kit/message"
...
function WebSearchQuick() {
  return (
    <a href="/dashboard/ai-center" className="rounded-full border border-gray-300 px-2 py-0.5 text-[11px] text-gray-700 bg-white hover:bg-gray-100">Web search</a>
  )
}
...
export default function SuitpaxAIPage() {
  ...
  const [includeReasoningInlineState, setIncludeReasoningInline] = useLocalStorage<boolean>("suitpax_ai_reasoning", true)
  ...
  const sendNonStreaming = async (userMessage: Message) => {
    const response = await fetch("/api/chat", {
      ...
      body: JSON.stringify({ message: userMessage.content, history: messages, includeReasoningInline }),
    })
    if (!response.ok) throw new Error("Failed to get response")
    const data = await response.json()
    const assistantMessage: Message = { id: (Date.now() + 1).toString(), content: data.response, role: "assistant", timestamp: new Date(), reasoning: data.reasoning, sources: data.sources || [] }
      ...
      <div className="fixed inset-0 flex flex-col">
        <div className="border-b border-gray-200">
          <div className="mx-auto w-full max-w-4xl px-4 py-3 flex items-center justify-between">
            ...
            <div className="flex items-center gap-2 text-[12px] text-gray-700">
              ...
              <Switch checked={includeReasoningInlineState} onCheckedChange={setIncludeReasoningInline} />
            </div>
          </div>
        </div>
        <ChatContainerRoot className="flex-1">
          <ChatContainerContent className={`mx-auto w-full max-w-3xl ${isSmall ? "px-2 py-3" : "px-4 py-6"} space-y-4`} role="feed" aria-label="Chat messages" aria-live="polite">
            ...
            {messages.length === 0 && isLoading && (
              <div className="py-16"><DashboardLoadingScreen className="bg-transparent" /></div>
            )}
            ...
            {messages.map((m, idx) => {
              ...
              return (
                <PKMessage key={m.id} className={`mx-auto flex w-full max-w-3xl flex-col gap-2 ${isSmall ? "px-0" : "px-0 md:px-6"} ${isAssistant ? "items-start" : "items-end"}`}>
                  {isAssistant ? (
                    <div className="group flex w-full flex-col gap-1">
                      <PKMessageContent className="text-foreground prose w-full flex-1 rounded-lg bg-transparent p-0" markdown>{m.content}</PKMessageContent>
                      {includeReasoningInlineState && m.reasoning && (
                        <div className="mt-1">
                          <Reasoning>
                            <ReasoningTrigger />
                            <ReasoningContent>
                      ...
                      <PKMessageActions className={`-ml-2.5 flex gap-0 opacity-0 transition-opacity duration-150 group-hover:opacity-100 ${isLast ? "opacity-100" : ""}`}>
                        ...
                        <PKMessageAction tooltip="Upvote">
                          <Button variant="ghost" size="icon" className="rounded-full"><ThumbsUp className="h-4 w-4" /></Button>
                        </PKMessageAction>
                        <PKMessageAction tooltip="Downvote">
                          <Button variant="ghost" size="icon" className="rounded-full"><ThumbsDown className="h-4 w-4" /></Button>
                        </PKMessageAction>
                      </PKMessageActions>
                    </div>
                  ) : (
                    <div className="group flex flex-col items-end gap-1">
                      <PKMessageContent className="bg-muted text-primary max-w-[85%] rounded-3xl px-5 py-2.5 sm:max-w-[75%]">{m.content}</PKMessageContent>
                    </div>
                  )}
                </PKMessage>
              )
            ...
            <div className="fixed right-6 bottom-24"><ScrollButton /></div>
          ...
          <div className={`mx-auto w-full max-w-4xl ${isSmall ? "px-2 py-3" : "px-4 py-4"}`}>
            <div className="pointer-events-none relative">
              <div className="pointer-events-auto">
                <PromptInput value={value} onValueChange={setValue} isLoading={isLoading || isStreaming} onSubmit={handleSend} aria-label="AI prompt input" className="shadow-md border border-gray-200 bg-white/90 backdrop-blur-sm">
                  ...
                  <PromptInputActions className={`mt-2 flex w-full items-center justify-between gap-2 ${isSmall ? "px-2 pb-2" : "px-3 pb-3"}`}>
                      ...
                      <Dialog>
                        ...
                        <DialogContent>
                          ...
                          <div className="grid grid-cols-1 gap-2">
                            <a href="/api/integrations/google/drive/auth" className="rounded-lg border border-gray-200 px-3 py-2 text-sm text-gray-800 hover:bg-gray-50 inline-flex items-center gap-2"><img src="https://www.google.com/s2/favicons?sz=64&domain_url=https://drive.google.com" alt="Drive" className="size-4 grayscale" /><span>Connect Google Drive</span></a>
                            <a href="/api/integrations/google/gmail/auth" className="rounded-lg border border-gray-200 px-3 py-2 text-sm text-gray-800 hover:bg-gray-50 inline-flex items-center gap-2"><img src="https://www.google.com/s2/favicons?sz=64&domain_url=https://mail.google.com" alt="Gmail" className="size-4 grayscale" /><span>Connect Gmail</span></a>
                            <a href="/api/integrations/google/calendar/auth" className="rounded-lg border border-gray-200 px-3 py-2 text-sm text-gray-800 hover:bg-gray-50 inline-flex items-center gap-2"><img src="https://www.google.com/s2/favicons?sz=64&domain_url=https://calendar.google.com" alt="Calendar" className="size-4 grayscale" /><span>Connect Calendar</span></a>
                          </div>
                        </DialogContent>
                      </Dialog>
                      <WebSearchQuick />
                    ...
                    <div className="ml-auto">
                      <PromptInputAction tooltip={isLoading || isStreaming ? "Stop" : "Send"}>
                        <Button size="icon" aria-label={isLoading || isStreaming ? "Stop generation" : "Send message"} disabled={!value.trim() || isLoading || isStreaming} onClick={handleSend} className={`${isSmall ? "h-9 w-9" : "h-10 w-10"} rounded-full bg-black text-white hover:bg-gray-900`}>
                          {!isLoading && !isStreaming ? <ArrowUp className="size-4" /> : <span className="size-3 rounded-xs bg-white" />}
                        </Button>
                      </PromptInputAction>
                    </div>
                  </PromptInputActions>
