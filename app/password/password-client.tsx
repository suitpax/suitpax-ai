...
export default function PasswordGatePageClient() {
	...
	const accessSchema = z.object({ password: z.string().min(6, "Access key must be at least 6 characters") })
	...
	const suitpaxLoadingLabels = ["Suitpax API", "Suitpax Flights", "Suitpax Rail", "Suitpax Ride", "Suitpax Hotels", "Suitpax Bank"]
	const [labelIndex, setLabelIndex] = useState(0)
	useEffect(() => {
		const id = setInterval(() => setLabelIndex((i) => (i + 1) % suitpaxLoadingLabels.length), 2200)
	...
	const handleSubmit = async (e: React.FormEvent) => {
		...
		try {
			...
			const res = await fetch("/api/password/verify", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ password }) })
	...
	const submitContact = async (e: React.FormEvent) => {
		...
		try {
			...
			const resp = await fetch("/api/contact", {
				...
				body: JSON.stringify({ email: contactEmail, company: contactCompany, message: contactMessage, name: "Website Visitor", subject: "Early access request" }),
			})
			const json = await resp.json()
			if (!resp.ok) throw new Error(json?.message || json?.error || "Message failed")
	...
	return (
		<VantaHaloBackground
			...
			options={{ backgroundColor: 0xeaf2ff, baseColor: 0xdbeafe, color: 0x93c5fd, color2: 0xbfe0ff, size: 0.8, amplitudeFactor: 0.7 }}
		>
			<div className="container mx-auto px-4 md:px-6 relative z-10 py-16 md:py-24">
				<div className="flex flex-col items-center text-center">
					...
					<motion.h1 initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }} className="text-4xl md:text-5xl lg:text-6xl font-serif tracking-tighter text-black leading-tight max-w-4xl">
						{headline}
					</motion.h1>
					...
					<div className="mt-2">
						<MiniCountdownBadge target={new Date("2025-10-21T00:00:00Z")} title="Official launch" variant="light" />
					</div>

					<div className="mt-3">
						<span className="inline-flex items-center gap-2 rounded-xl border border-lime-300 bg-lime-200/70 px-2.5 py-1 text-[10px] font-medium text-black">
							<span className="w-2 h-2 rounded-full bg-lime-500 animate-pulse" />
							<span>Loading {suitpaxLoadingLabels[labelIndex]}</span>
						</span>
					</div>
					...
					<form onSubmit={handleSubmit} className="mt-2 w-full max-w-xl mx-auto px-2">
						<div className="flex items-center gap-2 bg-white/90 backdrop-blur border border-gray-200 rounded-full p-1.5 shadow-sm">
							...
							<input type={showPassword ? "text" : "password"} value={password} onChange={(e) => setPassword(e.target.value)} placeholder="Enter access key to continue" className="flex-1 bg-transparent text-[12px] md:text-[13px] text-gray-900 placeholder:text-gray-500 focus:outline-none h-8" required aria-invalid={!!error} aria-describedby={error ? "password-error" : undefined} />
							<button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute right-10 inline-flex items-center justify-center text-gray-400 hover:text-gray-600" aria-label={showPassword ? "Hide access key" : "Show access key"} title={showPassword ? "Hide access key" : "Show access key"}>
								{showPassword ? (
							...
							<button type="submit" disabled={loading} className="h-7 w-7 md:h-8 md:w-8 rounded-full text-black hover:text-gray-700 flex items-center justify-center" aria-label="Continue" title="Continue">
					...
					<div className="mt-10 w-full max-w-2xl">
						...
						<form onSubmit={submitContact} className="space-y-3">
							<textarea value={contactMessage} onChange={(e) => setContactMessage(e.target.value)} placeholder="What do you want to achieve with Suitpax?" className="w-full bg-white/90 border border-gray-200 rounded-2xl p-3 text-[13px] text-gray-900 placeholder:text-gray-500 focus:outline-none backdrop-blur min-h-[120px]" rows={4} required aria-label="Message" />
							<div className="grid grid-cols-1 md:grid-cols-2 gap-2">
								<input type="email" value={contactEmail} onChange={(e) => setContactEmail(e.target.value)} placeholder="Work email" className="w-full bg-white/90 border border-gray-200 rounded-2xl p-2.5 text-[13px] text-gray-900 placeholder:text-gray-500 focus:outline-none backdrop-blur" required aria-label="Email" />
								<input value={contactCompany} onChange={(e) => setContactCompany(e.target.value)} placeholder="Company" className="w-full bg-white/90 border border-gray-200 rounded-2xl p-2.5 text-[13px] text-gray-900 placeholder:text-gray-500 focus:outline-none backdrop-blur" required aria-label="Company" />
							</div>
							<button type="submit" disabled={contactLoading} className="w-full md:w-auto md:self-end px-6 h-10 rounded-2xl bg-black text-white hover:bg-gray-900 text-[13px]">{contactLoading ? "Sending…" : "Request access"}</button>
						</form>
			...
			<div className="absolute inset-x-0 bottom-0 z-10">
				<div className="px-4 py-2 bg-black/95 text-gray-300 text-center text-[11px] font-medium tracking-tight">Private preview • Building the future of business travel</div>
				<div className="px-4 py-1 bg-black/95 text-center text-[10px] text-gray-400">Suitpax™ is a trademark of Suitpax. All product names, logos, and brands are property of their respective owners.</div>
			</div>
		</VantaHaloBackground>
	)
}